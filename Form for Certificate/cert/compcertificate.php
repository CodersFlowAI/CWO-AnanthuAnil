<?php
	...
	//Check Duplicates
	$duplicate_email = "SELECT email FROM certy WHERE email = '$email' AND certificateid = 2 ";
	...
	if($count > 0) {
		...
		if ($conn->query($sql) === TRUE) {
			...
			if($rescount > 0){
				...
				$font = "BRUSHSCI.TTF";
				$image = imagecreatefromjpeg('compcertificate.jpg');
				$color = imagecolorallocate($image, 0, 0, 0);
				
				//Format - imagettftext(image, size, angle, x, y, color, fontfile, text)
				//Adding Name 
				imagettftext($image, 65, 0, 300, 380, $color, $font, $name);

				//Adding Date
				$date = "6th may 2020";
				imagettftext($image, 30, 0, 90, 110, $color, $font, $date);
				...
				if ($idcount > 0) {
					...
					mysqli_query($conn,"UPDATE certy SET certificateid=2 WHERE id='" . $row['id'] . "'");

					//Send Image to Browser
					//imagejpeg($image); #Display Certificate on browser screen
					imagejpeg($image,"certificate/id-" . $row['id'] . " name-" . $name . " certid-2.jpg");
