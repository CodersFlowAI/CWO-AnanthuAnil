<?php
	...
	//Check Duplicates
	$duplicate_email = "SELECT email FROM certy WHERE email = '$email'  AND certificateid = 3 ";
	...
	if($count > 0) {
		...
		if ($conn->query($sql) === TRUE) {
			...
			if($rescount > 0){
				...
				$font = "BRUSHSCI.TTF";
				$image = imagecreatefromjpeg('interncertificate.jpg');
				$color = imagecolorallocate($image, 0, 0, 0);
				
				//Format - imagettftext(image, size, angle, x, y, color, fontfile, text)
				//Adding Name 
				imagettftext($image, 75, 0, 275, 410, $color, $font, $name);

				//Adding Date
				$date = "6th may 2020";
				imagettftext($image, 30, 0, 430, 775, $color, $font, $date);
				...
				if ($idcount > 0) {
					...
					mysqli_query($conn,"UPDATE certy SET certificateid=3 WHERE id='" . $row['id'] . "'");

					//Send Image to Browser
					//imagejpeg($image); #Display Certificate on browser screen
					imagejpeg($image,"certificate/id-" . $row['id'] . " name-" . $name . " certid-3.jpg");
